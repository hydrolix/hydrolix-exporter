...
1. **Use versioned tags** instead of `latest`
   ```yaml
   image: us-docker.pkg.dev/hdx-art/t/hdx-collector:v1.1.1
...
3. **Retry and queuing are enabled by default** in v1.1.1 with sensible values. Override only if you need to tune them:
   ```yaml
   exporters:
     hydrolix:
       # ... other config
